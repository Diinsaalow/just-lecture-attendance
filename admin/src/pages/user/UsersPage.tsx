...
/** Staff & admins on `/users` (instructors are managed under Lecturers). */
const UsersPage = () => {
        ...
        try {
            ...
            const msg =
                typeof error === 'object' && error !== null && 'data' in error
                    ? String((error as { data?: { message?: string } }).data?.message)
                    : '';
            toast.error(msg || 'Failed to delete users');
        }
    };

    const breadcrumbItems = [{ title: 'Dashboard', path: '/' }, { title: 'Users', path: '/users' }, { title: 'Staff accounts' }];
    ...
    const getRecordId = (record: IUser): number => parseInt(record._id.replace(/\D/g, '').slice(-8) || '0', 10) || 0;
